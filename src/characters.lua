...
function newCat()
    ...
    myCat.dimensions = 32
    myCat.sprite = love.graphics.newQuad(0, 0, myCat.dimensions, myCat.dimensions,
        myCat.spriteSheet.sheet:getDimensions())
...
function newZombie(x, y)
    ...
    zombie.sprite = love.graphics.newQuad(0, zombie.dimensions * 2, zombie.dimensions, zombie.dimensions,
        zombie.spriteSheet.sheet:getDimensions())
...
function moveZombie(zombie, dt)
    local centerX, centerY = (zombie.spriteDimension.width * zombie.scale2) / 2,
        (zombie.spriteDimension.height * zombie.scale2) / 2 -- get center point
...
function checkCatHit(myZombie, pos)
    ...
    if myZombie then
        if (myZombie.x + myZombie.dimensions * myZombie.scale2 >= cat.x and myZombie.x <= cat.x + cat.dimensions *
            cat.scale) and
