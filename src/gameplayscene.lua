...
function Gameplay_Init()
    ...
    pauseButton = newButton("PAUSE", pauseFont, padding, padding, 60, 30, pauseButtonColor, pauseHighlightColor,
        pauseTextColor)
...
function Gameplay_Update(dt)
    ...
    for i = 1, #zombies do
        if zombies[i].x ~= cat.x then
            if (zombies[i].x > cat.x + cat.dimensions and zombies[i].scale1 < 0) or
                (zombies[i].x < cat.x and zombies[i].scale1 > 0) then
...
function Gameplay_Draw()
    ...
    for i = 1, #zombies do
        love.graphics.draw(zombies[i].spriteSheet.sheet, zombies[i].sprite, zombies[i].x, zombies[i].y,
            zombies[i].angle, -zombies[i].scale1, zombies[i].scale2)
    end
